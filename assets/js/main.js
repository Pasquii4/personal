document.addEventListener('DOMContentLoaded', () => {
  ...
  const FALLBACK_REPOS = '22 públicos';
  ...
  const fetchGithubData = async () => {
    try {
      ...
      if (userData.public_repos !== undefined) {
        if (reposEl) reposEl.innerHTML = `${userData.public_repos} públicos ${renderLiveBadge()}`;
      } else {
      ...
      if (languages.size > 0) {
        if (langsEl) langsEl.innerHTML = `${languages.size} lenguajes ${renderLiveBadge()}`;
      } else {
